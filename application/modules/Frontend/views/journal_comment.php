<?php if(isset($journal_comment) && !empty($journal_comment)){
   if(count($journal_comment)>0){ ?>
<button type="submit" class="btn btn-submit-request mt-3 hide col-md-12 comment-view-btn" data-bs-toggle="collapse" href="#comment-journalviewcomment" role="button" aria-expanded="false" aria-controls="collapseExample">View Comment</button> 
<?php } ?> 
<div class="recent-item collapse show" id="comment-journalviewcomment">
   <?php echo lang('comment_text') ?>
   <?php foreach($journal_comment as $cval){ ?>
   <div class="comment-parent">
      <div class="r-i-head d-flex align-items-center position-relative w-100">
         ...
         <h3 class="comment-title">
            <b><?php echo user_detail_key($cval->id,'firstname').' '.user_detail_key($cval->id,'lastname'); ?></b> published a tribute <br>
   ...
   <?php }
    ?>
   <button id="loadMore_journolpost" type="button" class="btn btn-submit-request m-auto mt-3" data-postid="">Load More</button>
