         ...
         <table class="table table-striped table-bordered tablelist" id="userlist">
            ...
            <tbody>
               ...
               <tr>
                  ...
                  <td class="mobile-flex" data-header="<?php echo lang('description') ?>"><?php echo $post_val->media_caption ?></td>
                  <td class="mobile-flex" data-header="<?php echo lang('profile_name') ?>"><?php echo get_userprofile($post_val->profile_id)->profile_name ?></td>
                  <td class="mobile-flex" data-header="<?php echo lang('image') ?>"><img src="<?php echo UploadStorage::url($post_val->image, site_url( '/assets/frontend/uploads/' . get_defaultimage()->profile_img ) ); ?>" width="70" alt="<?php echo $post_val->title ?>" onerror="this.onerror=null; this.src='<?php echo base_url('assets/frontend/uploads/').get_defaultimage()->profile_img ?>'"/></td>
                  <td class="mobile-flex" data-header="<?php echo lang('date') ?>">
                     <?php echo $post_val->created_at ?>
                  </td>
                  <td class="mobile-flex" data-header="<?php echo lang('is_public') ?>"><?php echo ($post_val->media_ispublic==1) ? 'Public' : 'Private' ?></td>
                  ...
                  <td class="mobile-flex" data-header="<?php echo lang('action') ?>">
                     <div class="action-btnrow">
                        ...
                        <a  href="<?php echo site_url('updateactivestatus/media_images/'.$post_val->id.'/1') ?>" class="btn-primary btn"><?php echo lang('active') ?></a>
                        <?php }else{ ?>
                        <a  href="<?php echo site_url('updateactivestatus/media_images/'.$post_val->id.'/2') ?>" class="btn-danger btn"><?php echo lang('deactive') ?></a>
                        <?php } }
                        }else{
                           $nopermission = '';
                           echo lang('privacy_text');
                        } ?>
                        <a  href="<?php echo site_url('edit-albumimage/'.$post_val->id) ?>" class="btn-primary btn"><?php echo lang('edit_btn') ?></a>
                        <?php
                        if(get_frontauthuser('warden_user_id')==0 || (get_frontauthuser('warden_user_id') && checkwarden_permission(get_frontauthuser('warden_user_id'),'media_post','delete'))){
                           $nopermission = ''; ?>
                        <a  href="<?php echo site_url('delete_album_image/'.$post_val->id) ?>" onclick="return confirm('<?php echo lang('confirm_delete_msg') ?>')" class="btn-primary btn"><?php echo lang('delete') ?></a>
                        <?php } if($nopermission!=''){
                           echo $nopermission;
                        } ?>
                     </div>
