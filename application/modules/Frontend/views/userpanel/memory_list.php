            ...
            <tbody>
               ...
               <tr>
                  ...
                  <td class="mobile-flex" data-header="<?php echo lang('image') ?>">
                     <img src="<?php echo ($post_val->memoryimage!='') ? base_url('assets/frontend/uploads/').$post_val->memoryimage : base_url('assets/frontend/uploads/').get_defaultimage()->memory ?>" width="70" alt="<?php echo $post_val->name ?>" onerror="this.onerror=null; this.src='<?php echo base_url('assets/frontend/uploads/').get_defaultimage()->memory ?>'"/>
                  </td>
                  ...
                  <td class="mobile-flex" data-header="<?php echo lang('action') ?>">
                     <div class="action-btnrow">
                        ...
                        if(in_array($post_val->profile_id,get_userprofile_ids())){
                           $nopermission = '';
                        if(get_frontauthuser('warden_user_id')==0 || (get_frontauthuser('warden_user_id') && checkwarden_permission(get_frontauthuser('warden_user_id'),'memory_post','delete'))){
                           $nopermission = ''; ?>
                        <a href="<?php echo site_url('delete_memory/'.$post_val->id) ?>" onclick="return confirm('<?php echo lang('confirm_delete_msg') ?>')" class="btn-primary btn"><?php echo lang('delete') ?></a>
                        <?php }
                        if(get_frontauthuser('warden_user_id')==0 || (get_frontauthuser('warden_user_id') && checkwarden_permission(get_frontauthuser('warden_user_id'),'memory_post','approve'))){ ?>
                        <?php if($post_val->status==0 || $post_val->status==2){ ?>
                        <a  href="<?php echo site_url('updateactivestatus/memories/'.$post_val->id.'/1') ?>" class="btn-primary btn"><?php echo lang('active') ?></a>
                        <?php }else{ ?>
                        <a  href="<?php echo site_url('updateactivestatus/memories/'.$post_val->id.'/2') ?>" class="btn-danger btn"><?php echo lang('deactive') ?></a>
                        <?php } } }
                        if($nopermission!=''){
                           echo $nopermission;
                        } ?>
