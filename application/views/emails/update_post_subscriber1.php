                    ...
                    foreach($post_result as $key => $post_val){
                        ...
                        foreach($post_val as $val){
                         ...
                         //echo "<pre>";print_r($post_val);
                            ...
                            echo '<td>'.((isset($val->profile_id) && $val->profile_id!='') ? '<a style="display:inline-block;text-decoration: none;font-size: 15px;padding: 20px 35px; background-color:#ffffff; color:#90a792; font-family:Georgia,serif;border-radius: 10px;font-weight: 100;text-transform: uppercase;" href="'.site_url('?profile=').get_userprofile($val->profile_id)->profile_url.'">View</a>' : '').'</td>';
                            echo '</tr>';
                            }
                        }
                        // print_r($post_val);
