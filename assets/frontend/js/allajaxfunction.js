...
const baseurl = 'https://profile.memorisation.co.uk/';
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.status == true) {
                    ...
                    $('#media-album-name').html(data.album_data);
                    return false;
                } else {
                    //$('.'+form_mailid+'-error').removeClass('hide').html(data.message);
                    return false;
            ...
            if (data.status == true) {
                ...
                if (data.body.image_url && data.body.image_url != '') {
                    ...
                    if(explodefile[explodefile.length-1]=='mp4'){
                        ...
                        $('#' + popupid).find('.img-cover-life').html('<img src="'+data.body.image_url+'" class="w-100 life-modal-img" onerror="this.onerror=null; this.src="'+baseurl+'/assets/frontend/uploads/LifeoEarlyLife.png">');
        ...
        success: function (resp) {
            ...
            if (data.status == true) {
                ...
                if(explodefile[explodefile.length-1]=='mp4'){
                    ...
                    $('#' + popupid).find('.img-cover-life').html('<img src="'+img_name+'" class="w-100 life-modal-img" onerror="this.onerror=null; this.src="'+baseurl+'/assets/frontend/uploads/LifeoEarlyLife.png">');
                    //$('#' + popupid).find('.' + popupid + '-img').attr('src', data.body.image_url);
                }
...
function mediapost_popup(ele, id = 0, post_type,sliderkey = 0) {
    console.log('sliderkey',id, ele,post_type,sliderkey);
    const swiperConfiguration = {
        initialSlide: parseInt(sliderkey),
        init: true,
        loop: true,
        speed: 1000,
        autoplay: {
    ...
    const swiper = new Swiper(".mySwiper", swiperConfiguration);
            ...
            if (data.status == true) {
                ...
                if (data.body.image_url && data.body.image_url != '') {
                    ...
                    if(explodefile[explodefile.length-1]=='mp4'){
                        ...
                        this_popup.find('.journalpost-image').html('<img src="'+data.body.image_url+'" class="w-100 life-modal-img" onerror="this.onerror=null; this.src="'+baseurl+'/assets/frontend/uploads/LifeoEarlyLife.png">');
                        //$('#' + popupid).find('.' + popupid + '-img').attr('src', data.body.image_url);
                    }
                    // $('#' + popupid).find('.' + popupid + '-img').attr('src', data.body.image_url);
                }
...
function deletefeature_post(element, id, parent_class, posttype) {
    var mesg = 'Please confirm you wish to removing as a featured post';
    if (confirm(mesg)) {
        $.ajax({
...
function loginform(element) {
    ...
    var form_mailid = $(element).attr('id');
    var url_login = baseurl+"/user-login";
    console.log('loginform', email,url_login);
    $.ajax({
        url: url_login,
        type: 'POST',
...
$(document).ready(function () {
    ...
    $("#lifeof-formvalidate").validate({
        ...
        submitHandler: function (form) {
            ...
            // formdata.append('other_data',other_data);
            formdata.append('fileToUpload', fileToUpload);
            $(element).find('*[type=submit]').val('Updating...');
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.status == true) {
                        ...
                        // console.log('asdad',data);
                        $('#lifeof_loop').html(data.body);
                        $('.lifeofsection-sucess').removeClass('hide');
    ...
    $('#editjournalpostpopup2210-form').validate({
        rules: {
            title: {
                required:true,
                maxlength:30
    ...
    $('#shareMemory-form').validate({
        ...
        submitHandler: function (form) {
            ...
            $(element).find('button[name=submit]').text('images taking a while to upload...');
    ...
    $('#comment-timeline').validate({
        ...
        submitHandler: function (form) {
            ...
            console.log('form', other_data,user_id);
            $.ajax({
